...
class grocery_CRUD_Model  extends CI_Model  {
    ...
    function get_field_types_basic_table()
    {
      ...
      foreach($this->db->query("SELECT column_name as Field, data_type as Type,  is_nullable as Null,  case when ordinal_position = 1 then 'PRI' else '' end as Key,  column_default as Default,'' as Extra  FROM information_schema.columns WHERE table_name ='{$this->table_name}'")->result() as $db_field_type)
//foreach($this->db->query("SELECT column_name as Field FROM information_schema.columns WHERE table_name ='{$this->table_name}`")->result() as $db_field_type)
    	{
